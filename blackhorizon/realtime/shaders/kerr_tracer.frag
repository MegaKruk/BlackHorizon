...
// Interior (doomed observer) mode: rays are generated by local
// aberration from the infalling camera's orthonormal tetrad, carry a
// per-ray conserved energy p_t, are never captured at the horizon
// (crossing outward backward in time is how the infaller sees the
...
// Conserved covariant time momentum of the current ray. Exterior
// cameras normalize every ray to +1 (static-at-infinity frequency);
// interior tetrad cameras give each ray its own conserved energy.
float P_T = 1.0;
...
vec3 disk_emission(vec3 hit_pos, vec3 hit_p) {
    ...
    float nu_emitted = u_t * (P_T + dot(hit_p, v));
...
void main() {
    ...
    if (u_interior_mode == 1) {
        // Local camera-frame look direction: components along the
        // tetrad legs (forward, right, up), not world axes.
        vec3 local = normalize(vec3(
            1.0,
            ndc.x * u_tan_half_fov,
            ndc.y * u_tan_half_fov / u_aspect
        ));
        vec4 physical = u_tetrad_time
            - local.x * u_tetrad_forward
            - local.y * u_tetrad_right
            - local.z * u_tetrad_up;
        Geometry g0 = evaluate_geometry(pos);
        float l_dot = physical.x + dot(g0.l, physical.yzw);
        // Lower the index of -physical with g = eta + 2 H l l.
        P_T = physical.x - 2.0 * g0.h * l_dot;
        p = -physical.yzw - 2.0 * g0.h * l_dot * g0.l;
    } else {
        p = initial_momentum(pos, dir);
    }
    float capture_radius = u_interior_mode == 1
        ? u_interior_stop
        : u_horizon_radius * (1.0 + u_capture_margin);
    int status = 0;
    vec3 dx_final = dir;
